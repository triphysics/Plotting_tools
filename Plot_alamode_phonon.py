...
data = np.loadtxt('TiO2.bands')
kpath, kval = get_kpath_and_kval('TiO2.bands')
...
scph = np.loadtxt('SCPH_TiO2_3.scph_bands')
temperature = 20
...
ax.plot(scph[:,1], scph[:,2], linestyle='-', marker='None', ms=3, color='k', label='Anharmonic, T = %d K' % temperature)
ax.plot(scph[:,1], scph[:,3:], linestyle='-', marker='None', ms=3, color='k', label='_Harmonic')
